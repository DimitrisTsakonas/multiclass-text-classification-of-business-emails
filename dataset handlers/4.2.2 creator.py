"""
Purpose:
    Converts Version 4.2 monthly pkl files into Version 4.2.2: removes class
    C, keeps only the combined text column and the label, and encodes labels
    as numbers. Deliberately skips lowercasing and stemming, since this
    version exists to feed the BERT embedding generators (4.2.2.1/4.2.2.2),
    which expect raw, unprocessed text rather than TF-IDF-style preprocessing.

Thesis reference:
    Dataset preparation (Section 4.2.1) for the BERT embedding experiments
    (Experiments 4 and 6).
...
Outputs:
    One Version 4.2.2 .pkl file per month, saved alongside the input files
    under a sibling "Version 4.2.2 pkl cout" folder.
"""
...
def removec_numbers_func(data_folder_path="<path-to-dataset>/Version 4.2 pkl"):
    """takes 4.2 version of data as input and creates a new version 4.2.2 as a step for 4.2.2.1 to be created.
    1) Removes class C.
    2) Keeps only ['To From CC Subject Body'] and ['labels'].
    3) Encodes ['labels'] into numbers

    saves a .pkl file for each month in similar fashion with 4.2 but now its called 4.2.2
    ...
    for i, file_path in enumerate(tqdm(pkl_files, desc="Processing files")):
        ...
        new_file_path = file_path.replace("Version 4.2 pkl", "Version 4.2.2 pkl cout").replace(
            "version_4.2.pkl", "version_4.2.2.pkl")
