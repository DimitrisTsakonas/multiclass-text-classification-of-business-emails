"""
Purpose:
    Binary variant of tfidf_classifier.py: remaps labels to class 2
    (Disbursement Accounts) vs. everything else, then runs the same
    TF-IDF + feature-reduction + cumulative monthly fold pipeline as a
    one-vs-rest classifier. Includes its own main_caller() entry point.

Thesis reference:
    Not part of the 10 reported experiments. It is later,
    unfinished exploratory work (exp_number 87, well after the reported
    experiments) applying the "isolate one confusing class" idea used for
    classes 3/4 in Experiment 10 to class 2 instead.

Inputs:
    A folder of already-preprocessed monthly .pkl files (data_folder_path)
    with numeric labels and class C already removed.

Outputs:
    Per-fold .txt reports/.png plots and an all_months_results/ summary
    folder.
"""
...
def remap_labels_binary_class2_vs_rest(y):
    """
    One-vs-All: Class 2 → 1 (positive), All others → 0 (negative)
    """
    return [1 if label == 2 else 0 for label in y]
...
def first_stage_tfidf_classifier(exp_number, data_folder_path, param_results_folder, text_representation,
    ...
    for i, file_path in enumerate(pkl_files):
        if i + 1 < len(pkl_files):  # Ensures we stop before training on last file as no file to test it.
            ...
            y_train = remap_labels_binary_class2_vs_rest(y_train_original)  # encode labels to either 0 or 1
            ...
            y_test = remap_labels_binary_class2_vs_rest(y_test_original) # encode labels to either 0 or 1
            ...
            y_pred = classifier.predict(x_test_tfidf_reduced)  # this predicts 0 or 1

            # STEP 7: Evaluate the classifier
            accuracy = accuracy_score(y_test, y_pred)  # Calculate the accuracy of the classifier
            print("Validation Accuracy:", accuracy)
            report = classification_report(y_test, y_pred, target_names=["Not Class 2", "Class 2"])
            print(report)

            # Compute metrics # NOT MICRO
            ...
            with open(result_file_path, 'w') as result_file:
                ...
                result_file.write("\n\nConfusion Matrix: CLASS 2 = Positive (1). All other = (0)\n")
    ...
    with open(tp_fp_fn_txt_path, 'w') as file:
        ...
        file.write(f"Grouped per class: (CLASS 2 = Possitive (1), ALL OTHERS = negative (0)\n")
...
main_caller(exp_number="87",
            experiment_title="87_tfidf_xgboost_binary_2vsall_4_2_4",
