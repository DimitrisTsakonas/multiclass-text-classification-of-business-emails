"""
Purpose:
    TF-IDF classification pipeline with optional feature-count reduction:
    for each monthly cumulative fold, ranks TF-IDF features by mean score
    and keeps only the top (100 - pct_of_features_to_remove)%, trains a
    classifier on the reduced matrix, and tracks every misclassified
    instance (index, true/predicted label, and email content) for later
    analysis. Includes its own main_caller() entry point.

Thesis reference:
    Experiment 9 - Feature Reduction (Section 5.9), via
    pct_of_features_to_remove. The misclassified-instance tracking also
    feeds Experiment 8's analysis (see helper functions/
    missclassifications_handler.py) - this same script was reused across
    many experiment numbers by editing exp_number/model/data_folder_path
    and rerunning, not just the one active call below.

Inputs:
    A folder of already-preprocessed monthly .pkl files (data_folder_path)
    - labels are expected to already be numeric with class C already
    removed, since those steps happen upstream in dataset handlers/.

Outputs:
    Per-fold .txt reports/.png plots, an all_months_results/ summary
    folder, and a <exp_number>_misclassified_data.pkl file listing every
    misclassified test instance.
"""
...
main_caller(exp_number="75",
            experiment_title="75_tfidf_xgboost_99_5featremoval_4_2_4",
            ...
            pct_of_features_to_remove=99.5
            )
