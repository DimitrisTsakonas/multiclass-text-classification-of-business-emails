"""
Purpose:
    Evaluates the second stage of the Experiment 10 two-stage (meta-classifier)
    pipeline in isolation. Trains and tests a binary classifier on
    ground-truth class-3 (Fixtures) vs. class-4 (Negotiations) rows only (via
    filter_class_3_4), acting as the "specialist" that separates the combined
    class back into its original categories, using the same cumulative
    monthly-fold setup as the other tfidf_classifier* scripts.

Thesis reference:
    Section 5.10, Experiment 10 (Meta Classifier), Figures 5.19 & 5.20 ("the
    second stage classifier was then tested in isolation ... in a dataset
    containing only samples from class 3 and 4"). This script's current
    configuration (XGBoost, exp_number 83) matches the isolated-XGBoost
    comparison in Figure 5.20; an earlier Logistic Regression run (exp_number
    82) matches Figure 5.19. This covers only the second-stage-alone ablation
    of that experiment; the full chained two-stage cascade (first-stage
    predictions feeding this specialist) is implemented separately in
    tfidf_classifier_with_meta.py.

Inputs:
    Sorted monthly .pkl files (dict with 'Label' and
    'To From CC Subject Body' keys) in data_folder_path, filtered down to
    class 3/4 rows.
...
def filter_class_3_4(x, y):
    return [x[i] for i in range(len(y)) if y[i] in [3, 4]], [y[i] for i in range(len(y)) if y[i] in [3, 4]]


def tfidf_classifier(exp_number, data_folder_path, param_results_folder, text_representation, pct_of_features_to_remove, model, dataset="full",
                     stop_words=None, min_df="Not applicable", ngram_range="Not applicable"):
    ...
    for i, file_path in enumerate(pkl_files):
        if i + 1 < len(pkl_files):  # Ensures we stop before training on last file as no file to test it.
            ...
            x_train, y_train = filter_class_3_4(x_train, y_train)  # filter to keep only class 3 and 4
            y_train = [0 if label == 3 else 1 for label in y_train]  # map class 3,4 into 0,1
            ...
            x_test, y_test = filter_class_3_4(x_test, y_test)  # filter to keep only class 3 and 4
            y_test = [0 if label == 3 else 1 for label in y_test]  # map class 3,4 into 0,1
            ...
            y_pred = classifier.predict(x_test_tfidf_reduced)

            # STEP 7: Evaluate the classifier
            accuracy = accuracy_score(y_test, y_pred)  # Calculate the accuracy of the classifier
            print("Validation Accuracy:", accuracy)
            report = classification_report(y_test, y_pred, labels=[0, 1], target_names=["Class 3", "Class 4"])
            print(report)
            ...
            for class_id in [0, 1]:
                ...
                per_class_f1_dict[class_id + 3].append(f1)  # Store as 3/4 again for clarity in reports
            ...
            for class_idx in [3, 4]:
                tp = conf_matrix[class_idx-3, class_idx-3]  # because confusion matrix is now 2x2
                fp = conf_matrix[:, class_idx-3].sum() - tp
                fn = conf_matrix[class_idx-3, :].sum() - tp
...
def main_caller(exp_number, experiment_title, model, data_folder_path, desktop_results_folder, text_representation, pct_of_features_to_remove, dataset="full", ):
    ...
    tfidf_classifier(exp_number, data_folder_path, param_results_folder, text_representation, pct_of_features_to_remove, model=model, dataset=dataset,
                     stop_words="english", min_df=5, ngram_range=(1, 1))
...
main_caller(exp_number="83",
            experiment_title="83_tfidf_xgboost_binary_secondstage_4_2_4",
